...
function loadBVH(strBVH) {
  ...
  skeletonHelper = new THREE.SkeletonHelper(result.skeleton.bones[0]);
  console.log("스켈레톤 지오메트리 : ", skeletonHelper.geometry);
  skeletonHelper.skeleton = result.skeleton; // allow animation mixer to bind to THREE.SkeletonHelper directly
  console.log("리절트 스켈레톤 : ", result.skeleton);
  // result.skeleton.bones[0].boneTextureSize = 100;
  // result.skeleton.bones[0].boneTexture = new THREE.MeshBasicMaterial({
  //   color: 0xe80227,
  //   blending: 100,
  // });
  const boneContainer = new THREE.Group();
  ...
  const mesh = new THREE.SkinnedMesh(
    boneContainer.children[0].geometry,
    new THREE.MeshStandardMaterial({ color: 0xe80227, skinning: true })
  );
  console.log(mesh);
  scene.add(skeletonHelper);
  scene.add(boneContainer);
